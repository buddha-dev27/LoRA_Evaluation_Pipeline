...
For each strength value:
  For each prompt in prompts_zit.txt:
    For each seed multiplier (default: 1000, 2000, 3000):
      Queue one LoRA image at the specified strength
      Save to: lora_eval/zit/{character}/weight/{strength}/

The baseline folder (lora_eval/zit/{character}/baseline/) must already exist.
Run lora_eval_queue.py --baseline-only first if baseline images are missing.

REQUIREMENTS
------------
  - Python 3.x (no external packages required)
  - ComfyUI running and accessible at COMFY_HOST:COMFY_PORT
  - lora_evaluation_zit.json  (workflow, in same folder as this script)
  - prompts_zit.txt                   (one prompt per line, # = comment)
  - Baseline images already generated in lora_eval/zit/{character}/baseline/
    Run: python lora_eval_queue.py --character {character} --baseline-only
  - LoRA checkpoint file: {character}_zit_{step}.safetensors
...
-----
  ...
  python weight_queue_zit.py --dry-run

  # Full sweep — uses DEFAULT_CHARACTER and DEFAULT_CHECKPOINT set in variables
  python weight_queue_zit.py

  # Specify a different checkpoint
  python weight_queue_zit.py --checkpoint k41t1yn_zit_1700.safetensors

  # Different character — overrides DEFAULT_CHARACTER
  python weight_queue_zit.py --character k41t1yn

  # Custom strength range
  python weight_queue_zit.py --start 0.7 --end 0.9 --step 0.05

  # Second trigger word
  python weight_queue_zit.py --trigger2 redhead

  # Resume after interruption — N is the last completed image number from the log
  python weight_queue_zit.py --resume 15
...
DEFAULT_TRIGGER2   = ""
DEFAULT_CHECKPOINT = "k41t1yn_zit_2700.safetensors"
...
NODE_SEED_MULT   = 5433
NODE_PROMPT_ID   = 5392
NODE_PROMPT_TEXT = 5395
NODE_TRIGGER1    = 5387
NODE_TRIGGER2    = 5402
NODE_PATH_LORA   = 5404
NODE_PATH_BASE   = 5409
NODE_PATH_ROOT   = 5397
NODE_PATH_ROOT_B = 5407
NODE_LORA_LOADER = 5465   # LoraLoader
NODE_FILENAME    = 5466   # easy string: filename prefix
...
def parse_args():
    ...
    p.add_argument("--checkpoint",     default=DEFAULT_CHECKPOINT,
                   help="LoRA filename e.g. k41t1yn_zit_3500.safetensors")
