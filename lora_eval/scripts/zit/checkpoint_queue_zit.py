...
------------
  ...
  - Workflow file: lora_evaluation_zit.json (in same folder as this script)
  - Prompts file:  prompts_zit.txt (one prompt per line, # lines are comments)
  - LoRA files named: {character}_zit_{step}.safetensors
...
-----
  ...
  python checkpoint_queue_zit.py --dry-run

  # Full sweep — scans LoRA folder automatically, uses set variables
  python checkpoint_queue_zit.py

  # Different character — overrides DEFAULT_CHARACTER
  python checkpoint_queue_zit.py --character k41t1yn

  # Specific checkpoints only
  python checkpoint_queue_zit.py --checkpoints 1400 1500 1600

  # Range of checkpoints (start end step)
  python checkpoint_queue_zit.py --range 1400 2600 100

  # Use the short prompt list
  python checkpoint_queue_zit.py --prompts prompts_zit_short.txt

  # Second trigger word
  python checkpoint_queue_zit.py --trigger2 redhead

  # Baseline images only — use if needed only for weight evaluation
  python checkpoint_queue_zit.py --baseline-only

  # Resume after interruption — N is the last completed pair number from the log
  python checkpoint_queue_zit.py --checkpoints 2500 2600 2700 --resume 47
...
COMFY_OUTPUT_ROOT = Path(r"G:\output\lora_eval\zit")

# Log file location
LOG_ROOT = Path(r"G:\output\lora_eval\zit")

# ── Node IDs ────────────────────────────────────────────────────────────────
NODE_SEED_MULT   = 5433   # easy string: seed multiplier e.g. "1000"
NODE_PROMPT_ID   = 5392   # easy string: prompt ID e.g. "01"
NODE_PROMPT_TEXT = 5395   # easy string: prompt text
NODE_TRIGGER1    = 5387   # easy string: trigger word 1
NODE_TRIGGER2    = 5402   # easy string: trigger word 2
NODE_PATH_LORA   = 5404   # easy string: lora subfolder "/lora/"
NODE_PATH_BASE   = 5409   # easy string: base subfolder "/base/"
NODE_PATH_ROOT   = 5397   # easy string: root folder (lora branch) "lora_eval/"
NODE_PATH_ROOT_B = 5407   # easy string: root folder (base branch) "lora_eval/"
NODE_LORA_LOADER = 5465   # LoraLoader — lora_name patched per checkpoint
NODE_FILENAME    = 5466   # easy string: filename prefix e.g. "01_1000"

UUID_CLASS_MAP = {}

FALLBACK_NODES = {"LoraLoader", "LoraLoaderModelOnly"}
