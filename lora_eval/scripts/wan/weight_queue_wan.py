...
For each strength value:
  For each prompt in prompts_wan.txt:
    For each seed multiplier (default: 1000, 2000, 3000):
      Queue one LoRA image at the specified strength
      Save to: lora_eval/wan/{character}/weight{step}/{strength}/

The baseline folder (lora_eval/wan/{character}/baseline/) must already exist.
Run checkpoint_queue_wan.py --baseline-only first if baseline images are missing.

REQUIREMENTS
------------
  - Python 3.x (no external packages required)
  - ComfyUI running and accessible at COMFY_HOST:COMFY_PORT
  - lora_evaluation_wan.json  (workflow, in same folder as this script)
  - prompts_wan.txt                    (one prompt per line, # = comment)
  - Baseline images already generated in lora_eval/wan/{character}/baseline/
    Run: python checkpoint_queue_wan.py --character {character} --baseline-only
  - LoRA checkpoint file: {character}_wan_{step}_high_noise.safetensors
...
-----------------------------------
  ...
  LORA_SUBROOT        Subfolder within loras root     e.g. wan
  LORA_MODEL_TAG      Short model name in filenames   e.g. wan
  SEED_MULTIPLIERS    List of seed multipliers        default: [1000, 2000, 3000]
  OUTPUT_ROOT         ComfyUI-relative output root    e.g. lora_eval/wan
...
-----
  ...
  python weight_queue_wan.py --dry-run

  # Full sweep — uses DEFAULT_CHARACTER and DEFAULT_CHECKPOINT set in variables
  python weight_queue_wan.py

  # Specify a different checkpoint
  python weight_queue_wan.py --checkpoint k41t1yn_wan_1900_high_noise.safetensors

  # Different character — overrides DEFAULT_CHARACTER
  python weight_queue_wan.py --character k41t1yn

  # Custom strength range
  python weight_queue_wan.py --start 0.7 --end 0.9 --step 0.05

  # Second trigger word
  python weight_queue_wan.py --trigger2 k3nz13

  # Resume after interruption — N is the last completed image number from the log
  python weight_queue_wan.py --resume 15

ARGUMENTS
---------
  --character     Character name (trigger word 1, folder name)
  --checkpoint    LoRA filename e.g. k41t1yn_wan_1900_high_noise.safetensors
...
DEFAULT_TRIGGER2   = ""
DEFAULT_CHECKPOINT = "k41t1yn_wan_2700_high_noise.safetensors"
...
HIDDEN_WIDGETS = {
    ...
    "KSamplerAdvanced": [(2, "control_after_generate")],
}
...
def lora_comfy_path_ln(character, filename):
    """Low noise LoRA path — replaces _high_noise with _low_noise."""
    ln_stem = filename.replace("_high_noise", "_low_noise")
    return f"{LORA_SUBROOT}\\{character}\\{ln_stem}"
...
def parse_args():
    ...
    p.add_argument("--checkpoint",     default=DEFAULT_CHECKPOINT,
                   help="LoRA filename e.g. k41t1yn_wan_1900_high_noise.safetensors")
...
def main():
    ...
    _ckpt_m = re.search(r'_(\d+)(?:_(?:high|low)_noise)?\.safetensors$', args.checkpoint)
    ...
    lora_path_ln = lora_comfy_path_ln(character, args.checkpoint)
    ...
    _m = re.search(r'_(\d+)(?:_(?:high|low)_noise)?\.safetensors$', args.checkpoint)
        ...
        if not args.dry_run:
            ...
            for prompt_id, prompt_text in prompts:
                ...
                for multiplier in args.multipliers:
                    ...
                    NODE_LORA_LOADER_LN: lora_comfy_path_ln(character, args.checkpoint),
        ...
        for prompt_id, prompt_text in prompts:
            ...
            for multiplier in args.multipliers:
                ...
                patches = {
                    ...
                    NODE_LORA_LOADER_LN: lora_path_ln,
                    NODE_FILENAME:    f"{prompt_id}_{seed}",
                }

                wf = patch_ui_workflow(wf_ui_src, patches)
                patch_lora_strength(wf, NODE_LORA_LOADER, strength)
                patch_lora_strength(wf, NODE_LORA_LOADER_LN, strength)
                api = clean_api(ui_to_api(wf))
