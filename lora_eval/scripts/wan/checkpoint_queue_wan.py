...
------------
  ...
  - Workflow file: lora_evaluation_wan.json (in same folder as this script)
  - Prompts file:  prompts_wan.txt (one prompt per line, # lines are comments)
  - LoRA files named: {character}_wan_{step}_high_noise.safetensors
...
-----
  ...
  python checkpoint_queue_wan.py --dry-run

  # Full sweep — scans LoRA folder automatically, uses set variables
  python checkpoint_queue_wan.py

  # Different character — overrides DEFAULT_CHARACTER
  python checkpoint_queue_wan.py --character k41t1yn

  # Specific checkpoints only
  python checkpoint_queue_wan.py --checkpoints 1400 1500 1600

  # Range of checkpoints (start end step)
  python checkpoint_queue_wan.py --range 1400 2600 100

  # Use the short prompt list
  python checkpoint_queue_wan.py --prompts prompts_wan_short.txt

  # Second trigger word
  python checkpoint_queue_wan.py --trigger2 redhead

  # Baseline images only — use if needed only for weight evaluation
  python checkpoint_queue_wan.py --baseline-only

  # Resume after interruption — N is the last completed pair number from the log
  python checkpoint_queue_wan.py --checkpoints 1400 1500 1600 --resume 47
...
WORKFLOW_FILE = "lora_evaluation_wan.json"
PROMPTS_FILE  = "prompts_wan_short.txt"
...
COMFY_OUTPUT_ROOT = Path(r"G:\output\lora_eval\wan")

# Log file location
LOG_ROOT = Path(r"G:\output\lora_eval\wan")

# ── Node IDs ────────────────────────────────────────────────────────────────
NODE_SEED_MULT   = 204    # easy string: seed multiplier e.g. "1000"
NODE_PROMPT_ID   = 203    # easy string: prompt ID e.g. "01"
NODE_PROMPT_TEXT = 202    # easy string: prompt text
NODE_TRIGGER1    = 200    # easy string: trigger word 1
NODE_TRIGGER2    = 201    # easy string: trigger word 2
NODE_PATH_LORA   = 199    # easy string: lora subfolder e.g. "/checkpoint/1000/"
NODE_PATH_BASE   = 199    # easy string: base subfolder "/baseline/"
NODE_PATH_ROOT   = 198    # easy string: root folder "lora_eval/krea/"
NODE_PATH_ROOT_B = 198    # easy string: root folder (base branch)
NODE_LORA_LOADER    = 192    # LoraLoaderModelOnly High Noise
NODE_LORA_LOADER_LN = 193    # LoraLoaderModelOnly Low Noise
NODE_FILENAME       = 217    # easy string: File Name Prefix — patched with "{prompt_id}_{seed}"    # easy string: filename prefix e.g. "01_1000"
NODE_CHAR_NAME   = 200    # easy string: character name for output path e.g. "k41t1yn/"
...
FALLBACK_NODES = {"VHS_SelectEveryNthImage", "LoraLoader", "LoraLoaderModelOnly"}
...
def scan_checkpoints(character):
    ...
    pattern = re.compile(
        rf"^{re.escape(character)}_{re.escape(LORA_MODEL_TAG)}_(\d+)_high_noise\.safetensors$",
        re.IGNORECASE
    )
    ...
    if not found:
        ...
        log(f"  Expected: {character}_{LORA_MODEL_TAG}_NNNN_high_noise.safetensors")
        log(f"\n  Files found:")
...
def lora_comfy_path_ln(character, stem):
    """Low noise LoRA — replaces _high_noise with _low_noise in stem."""
    ln_stem = stem.replace("_high_noise", "_low_noise")
    return f"{LORA_SUBROOT}\\{character}\\{ln_stem}.safetensors"
...
HIDDEN_WIDGETS = {
    ...
    "KSamplerAdvanced": [(2, "control_after_generate")],  # noise_seed at pos 1 is linked; hidden widget is control_after_generate at pos 2
}
...
def patch_lora_strength(wf, node_id, strength):
    """Patch LoraLoader or LoraLoaderModelOnly strength widgets.
    LoraLoader has 3 widgets:      [lora_name, strength_model, strength_clip]
    LoraLoaderModelOnly has 2:     [lora_name, strength_model]
    """
    node_map = {n["id"]: n for n in wf["nodes"]}
    n = node_map.get(node_id)
    if not n:
...
def ui_to_api(wf_ui):
    ...
    for node in wf_ui.get("nodes", []):
        ...
        # Some nodes (e.g. VHS) store widgets_values as a dict
        if isinstance(wv_raw, dict):
            wv = list(wv_raw.values())
        else:
            wv = wv_raw
        inputs = {}
...
def main():
    ...
    if args.checkpoints:
        ...
        checkpoints = list(all_checkpoints.items())
        ...
        for prompt_id, prompt_text in prompts:
            ...
            for multiplier in args.multipliers:
                ...
                patches = {
                    ...
                    NODE_LORA_LOADER_LN: lora_comfy_path_ln(character, first_stem),
        ...
        for prompt_id, prompt_text in prompts:
            ...
            for multiplier in args.multipliers:
                ...
                patches = {
                    ...
                    NODE_LORA_LOADER_LN: lora_comfy_path_ln(character, stem),
                    NODE_FILENAME:    f"{prompt_id}_{seed}",
                }
                ...
                patch_lora_strength(wf, NODE_LORA_LOADER_LN, CHECKPOINT_LORA_STRENGTH)
                api = clean_api(ui_to_api(wf))
                pid = queue_prompt(args.host, args.port, api, client_id)
